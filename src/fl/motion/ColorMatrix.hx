...
class ColorMatrix extends DynamicMatrix {
	 ...
	 * @private
	 */
	private static inline var LUMINANCER:Float = 0.3086;

	/**
	 * @private
	 */
	private static inline var LUMINANCEG:Float = 0.6094;

	/**
	 * @private
	 */
	private static inline var LUMINANCEB:Float = 0.0820;
	 ...
	 * @param value 0-255
	 */
	public function SetBrightnessMatrix(value:Float):Void {
		if (m_matrix == null) {
			return;
		}
	 ...
	 * @param value 0-255
	 */
	public function SetContrastMatrix(value:Float):Void {
		if (m_matrix == null) {
			return;
		}

		var brightness:Float = 0.5 * (127.0 - value);
		value = value / 127.0;
	 ...
	 * @param value 0-255
	 */
	public function SetSaturationMatrix(value:Float):Void {
		if (m_matrix == null) {
			return;
		}

		var subVal:Float = 1.0 - value;

		var mulVal:Float = subVal * LUMINANCER;
	 ...
	 * @param value 0-255
	 */
	public function SetHueMatrix(angle:Float):Void {
		if (m_matrix == null) {
			return;
		}
		...
		var cosValue:Float = Math.cos(angle);
		var sinValue:Float = Math.sin(angle);
	 ...
	 */
	public function GetFlatArray():Array<Float> {
		if (m_matrix == null) {
			return null;
		}

		var ptr:Array<Float> = new Array<Float>();
		var index:Int = 0;
		for (i in 0...4) {
			for (j in 0...5) {
