...
class Animator extends AnimatorBase {
	 ...
	 * @private (setter)
	 */
	override private function set_motion(value:MotionBase):MotionBase {
		super.motion = value;
		var classicMotion:Motion = try cast(value, Motion) catch (e:Dynamic) null;
		if (classicMotion != null && classicMotion.source && classicMotion.source.transformationPoint) {
	 ...
	 * @private
	 */
	override private function setTimeClassic(newTime:Int, thisMotion:MotionBase, curKeyframe:KeyframeBase):Bool {
		var thisMotionClassic:Motion = try cast(thisMotion, Motion) catch (e:Dynamic) null;
		if (thisMotionClassic == null) {
			return false;
		}

		var matrix:Matrix = thisMotionClassic.getMatrix(newTime);
		if (matrix != null) {
			if (motionArray == null || _lastMatrixApplied == null || !matricesEqual(matrix, _lastMatrixApplied)) {
			...
			if (motionArray != null && thisMotionClassic != _lastMotionUsed) {
				this.transformationPoint = ((thisMotionClassic.motion_internal ::transformationPoint)) ? thisMotionClassic.motion_internal : : transformationPoint:new Point(.5,
					.5);
				initTransformPointInternal(thisMotionClassic.motion_internal::initialMatrix);
				_lastMotionUsed = thisMotionClassic;
			}

			var positionX:Float = thisMotionClassic.getValue(newTime, Tweenables.X);
			var positionY:Float = thisMotionClassic.getValue(newTime, Tweenables.Y);
			...
			var scaleX:Float = thisMotionClassic.getValue(newTime, Tweenables.SCALE_X) * this.targetState.scaleX;
			var scaleY:Float = thisMotionClassic.getValue(newTime, Tweenables.SCALE_Y) * this.targetState.scaleY;
			var skewX:Float = 0;
			var skewY:Float = 0;

			// override the rotation and skew in the XML if orienting to path
			if (this.orientToPath) {
				var positionX2:Float = thisMotionClassic.getValue(newTime + 1, Tweenables.X);
				var positionY2:Float = thisMotionClassic.getValue(newTime + 1, Tweenables.Y);
			...
			if (thisMotionClassic.useRotationConcat(newTime)) {
				...
				var rotConcat:Float = thisMotionClassic.getValue(newTime, Tweenables.ROTATION_CONCAT);
			...
			if (motionArray == null || _lastMatrixApplied == null || !matricesEqual(targetMatrix, _lastMatrixApplied)) {
	 ...
	 */
	public static function matricesEqual(a:Matrix, b:Matrix):Bool {
